...
class LMLP(nn.Module):  # Implements a L-bounded MLP with N sandwich layers
    def __init__(self, N, in_features, mid_features, out_features, bias=True, scale=1.0):
        super().__init__()
        self.fc = FirstChannel(in_features)
        self.Sand = nn.ModuleList(
            [SandwichFc(in_features, mid_features, bias, scale) for j in range(N)])
        self.linout = SandwichLin(mid_features, out_features, bias)
        self.Sand.insert(0, self.linin)
        self.Sand.append(self.linout)
        self.model = nn.Sequential(*self.Sand)
